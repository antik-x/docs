...
端到端运行以本地为先。当你登录了 [Confident AI](https://www.confident-ai.com)（`deepeval` 团队打造的平台）后，同一次运行还会生成可分享的云端[测试报告](https://www.confident-ai.com/docs/llm-evaluation/dashboards/testing-reports)，供团队一起评审，支持超参数对比和随时间的回归追踪——无需改代码，且完全可选。
</Accordion>
</AccordionGroup>
