...
Use `PromptOptimizer` when you would otherwise run evals, eyeball failures, and manually tweak prompts in a loop. It scores your [metrics](/docs/metrics-introduction) against a set of [goldens](/docs/evaluation-datasets#what-are-goldens) and uses those scores to automatically search for a higher-scoring prompt, so you get a data-driven rewrite instead of guesswork.
</Accordion>
<Accordion title="What are the required parameters to create a PromptOptimizer?">
There are two required parameters: `metrics` , a list of DeepEval metrics used for scoring and feedback, and `model_callback` , a callback that wraps your LLM app. The optional parameters are `algorithm` , `async_config` , `display_config` , and `mutation_config` .
</Accordion>
<Accordion title="What does my model_callback receive and have to return?">
During optimization DeepEval passes your callback the current `Prompt` candidate and a `Golden` (or `ConversationalGolden` ). You interpolate with `prompt.interpolate(input=golden.input)` , call your app, and the callback **must** return a `str` . That string is used as the `actual_output` in a [test case](/docs/evaluation-test-cases) before scoring.
</Accordion>
<Accordion title="Which optimization algorithm does DeepEval use by default?">
The default `algorithm` is `GEPA()` . DeepEval also ships [MIPROv2](/docs/prompt-optimization-miprov2) for surrogate-based search; pass an instance to the `algorithm` argument to switch, or to override algorithm-specific settings such as GEPA's `iterations` .
</Accordion>
<Accordion title="How do I run a prompt optimization asynchronously?">
Call `a_optimize()` , the async counterpart of `optimize()` , from within an async function and run it with `asyncio.run(...)` . This lets you run optimizations concurrently without blocking the main thread.
</Accordion>
<Accordion title="What is the optimization_report and when do I need it?">
The `optimization_report` is accessible on the `PromptOptimizer` instance and exposes six fields like `optimization_id` , `best_id` , `accepted_iterations` , and `parents` . Most workflows just use `optimized_prompt.text_template` (or `messages_template` ), but the report is useful for reconstructing the search tree or debugging why a configuration was selected as `best_id` .
</Accordion>
<Accordion title="How do I avoid rate-limit errors during optimization?">
Pass an `AsyncConfig` to `PromptOptimizer` and tune `throttle_value` and `max_concurrent` (defaulted to `20` ). Both only apply when `run_async` is `True` ; lowering `max_concurrent` and raising `throttle_value` is the recommended way to handle rate limits in your judge or LLM app.
</Accordion>
</AccordionGroup>
