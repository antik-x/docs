...
`.txt` , `.docx` , `.pdf` , `.md` , `.markdown` , and `.mdx` files, passed as a list to the `document_paths` parameter.
</Accordion>
<Accordion title="How are contexts constructed from my documents?">
`deepeval` parses documents into token-based chunks (via `chunk_size` and `chunk_overlap` ), selects and quality-filters chunks, then groups semantically similar chunks by cosine similarity into contexts. Tune this with a `ContextConstructionConfig` .
</Accordion>
<Accordion title="When should I use this instead of generate_goldens_from_contexts?">
Use `generate_goldens_from_docs` when you have raw documents and want `deepeval` to extract contexts for you. If you already control chunking, use [generate_goldens_from_contexts](/docs/synthesizer-generate-from-contexts) instead.
</Accordion>
