...
- [Optional] `model`: an OpenAI model name or a [custom evaluation model](/docs/metrics-introduction#using-a-custom-llm) of type `DeepEvalBaseLLM`. Defaulted to `gpt-5.4`.
...
- [Optional] `turn_window`: two inclusive indices restricting the node to a slice of the conversation.
