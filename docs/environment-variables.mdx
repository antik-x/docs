...
| `CONFIDENT_REGION`           | <Tabs><Tab title="Python">`US` / `EU` / unset</Tab><Tab title="TypeScript">`US` / `EU` / `AU` / unset</Tab></Tabs> | Confident AI data region. When unset, the region is inferred from your API key prefix.                                                                        |
...
（TypeScript：`GEMINI_API_KEY` is also accepted as an alias of `GOOGLE_API_KEY`.）
...
（TypeScript：`MOONSHOT_BASE_URL` overrides Moonshot's base URL.）
