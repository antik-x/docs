...
`MIPROv2` jointly optimizes the instruction and the few-shot demonstrations of a prompt, using Bayesian Optimization to search the space of (instruction, demo set) combinations. Use it when few-shot examples matter for your task or when you have a large candidate space you want to explore efficiently.
</Accordion>
<Accordion title="What do I need to install to run MIPROv2?">
MIPROv2 requires the `optuna` package for Bayesian Optimization. Install it with `pip install optuna` before constructing a `MIPROV2` instance.
</Accordion>
...
Bootstrapped demos are model-generated outputs that passed all your metrics, capped by `max_bootstrapped_demonstrations` (default `4` ). Labeled demos come from the `expected_output` of your [goldens](/docs/evaluation-datasets#what-are-goldens) , capped by `max_labeled_demonstrations` (default `4` ). MIPROv2 mixes both into `num_demonstration_sets` sets and always includes a 0-shot option.
</Accordion>
...
Pass `random_state` as either an `int` seed or a `random.Random` instance. It affects candidate generation, demo bootstrapping, minibatch sampling, and TPE sampling, so a fixed seed gives identical results.
</Accordion>
</AccordionGroup>
