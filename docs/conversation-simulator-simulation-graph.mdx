...
When the *trajectory* matters — deterministic ordering, retry budgets, branching on assistant behavior, or terminal success/failure states. The default single-prompt user is fine for fuzzy exploration, but a `simulation_graph` lets you encode a specific path like "ask, push back, escalate after three pushbacks, accept any compromise".
</Accordion>
<Accordion title="How do I make the simulation stop from inside the graph?">
Use `SimulationNode(terminal=True)` to emit one final user turn, record the assistant reply, then end — or set `max_visits=N` to cap how many times a node fires (the (N+1)-th entry ends the run with no new turn). Pair `max_visits` with a self-loop for "complain up to N times, then give up".
</Accordion>
<Accordion title="How does the graph decide which node runs next?">
After each assistant reply, `simulator_model` classifies it against the current node's outgoing `when=` edge descriptions (one LLM call, with a "None of the above" option appended) and advances to the matching child, staying put if nothing matches. A node with no edges loops on itself.
</Accordion>
<Accordion title="Can the stopping_controller end a graph-driven simulation early?">
Yes — the [stopping_controller](/docs/conversation-simulator-stopping-logic) runs *before* every user turn (including the first) and can end the run before a `terminal` node is ever reached. Both terminators can be active in the same run, and whichever fires first wins.
</Accordion>
</AccordionGroup>
