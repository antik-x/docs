...
- **Evaluation model**: the model deepeval will use when a metric (or synthesizer, simulator, etc.) is constructed without an explicit model, plus the reason it won. The default embedding model is listed alongside it, and the selected text-to-speech and speech-to-text models below those, with what each is used by.
...
`deepeval view` opens the latest test run on Confident
...
| Option          | Description                                                          |
| --------------- | -------------------------------------------------------------------- |
| `REGION`        | Positional argument. <Tabs><Tab title="Python">One of `US` or `EU`.</Tab><Tab title="TypeScript">One of `US`, `EU`, or `AU`.</Tab></Tabs> |
...
（TypeScript：Every `USE_*` flag here belongs to the LLM family, so setting one provider turns off all the others. With none set, `deepeval` falls back to OpenAI.）
