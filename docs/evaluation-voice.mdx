...
Your agent's speech stack is *inside* the system under test. `deepeval` 's speech models sit on the other side of the call — they are the simulated caller's mouth and ears, speaking user turns to your agent and transcribing what it says back so metrics can judge the conversation.
</Accordion>
<Accordion title="Which connector should I use to reach my agent?">
Use the connector matching how your agent is already deployed: `ElevenLabsConnector` for ElevenLabs conversational agents, `LiveKitConnector` for LiveKit rooms, or `WebSocketConnector` for a compatible raw-audio WebSocket. For an agent callable in the same Python process, use `CallbackVoiceConnector` .
</Accordion>
<Accordion title="What exactly does latency_ms measure?">
The time from sending the simulated user's audio to capturing the agent's spoken reply. The exact timing boundary depends on the connector's transport, so compare latency measurements made through the same connector type.
</Accordion>
<Accordion title="Do my existing multi-turn metrics work on voice conversations?">
Yes. A voice simulation produces a standard `ConversationalTestCase` whose transcript lives in each `Turn.content` — metrics like `TurnRelevancyMetric` judge it exactly as they would a text conversation. The audio and `latency_ms` fields are additional signal on top.
</Accordion>
<Accordion title="Can one connector serve multiple conversations concurrently?">
No — a connector holds a single live call, so the simulator connects before each conversation's first turn, disconnects when it ends, and runs conversations sequentially. Concurrent conversations would interleave audio on the same session.
</Accordion>
</AccordionGroup>
