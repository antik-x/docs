...
| 连接器                | 当你的代理是...                     | 你需要提供                                                |
| ------------------------ | ------------------------------------------------ | --------------------------------------------------------------- |
| `ElevenLabsConnector`    | ElevenLabs 会话代理               | 其 agent ID；私有代理还需 API key               |
| `LiveKitConnector`       | 运行在 LiveKit 房间中                        | LiveKit URL、API key、API secret，以及可选的代理派发 |
| `WebSocketConnector`     | 通过兼容的原始音频 WebSocket 暴露 | URL 和你的消息结构                                 |
| `CallbackVoiceConnector` | 可在同一 Python 进程内直接调用   | 接受用户音频并返回代理回复的可调用对象  |
